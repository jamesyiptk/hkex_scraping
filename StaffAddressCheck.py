...
try:
    ...
    data_structure = pd.DataFrame(columns=('Name', 'Result', 'Address', 'CaseID'))

    intindex=0
    for index, row in inhouse_address_dataframe.iterrows():

        staff_address = str(row["住址"])
        staff_address_phase = str(row["期數"])
        staff_address_block = str(row["座數"])
        staff_name = row["姓名"]
        ...
        for table in table_list:
            for row in table:
                ...
                if str(row[1]).find(staff_address) != -1:
                    ...
                    '''
    # Begin to Process compulsorytestingnotice
    # url = "https://www.chp.gov.hk/files/pdf/building_list_chi.pdf"
    ur11 = "https://www.coronavirus.gov.hk/pdf/compulsorytestingnotice_premises_CHI.pdf"
    r1 = requests.get(ur11, allow_redirects=True)
    download_file_path = download_file_path.replace(".pdf", "compulsorytestingnotice_premises_CHI.pdf")
    open(download_file_path, 'wb').write(r1.content)
    pdf = pdfplumber.open(download_file_path)
    ...
    for index, row in inhouse_address_dataframe.iterrows():
        ...
        if staff_address == "nan":
            continue
        if PDFText.find(staff_address) != -1:
            data_structure.loc[intindex] = {'Name': staff_name,
                                            'Result': 'Matched',
                                            'Address': staff_address,
                                            'CaseID': str(row[1])
