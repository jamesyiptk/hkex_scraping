...
download_file_2_csv_path = sys.argv[2]
input_address_path = sys.argv[3]
...
try:
    ...
    for index, row in inhouse_address_dataframe.iterrows():
      ...
      with open(download_file_2_csv_path, newline='',) as csvfile:
          rows_gov = csv.reader(csvfile)
          for row_gov in rows_gov:
              intindex=intindex+1
              if str(row_gov[1]).find(staff_address)==-1:
                  continue;
              else:
